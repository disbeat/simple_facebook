...
public class SimpleSearchView extends SearchView {

    public SimpleSearchView(IController c) {
        super(c);
    }
    ...
    public List<Item> searchItems(User user, String query) {
        try {
            return ItemManager.getInstance().searchItem(query, 20, null);
    ...
    public List<User> searchPeople(User user, String query) {
        try {
            return UserManager.getInstance().searchUser(query, 20, null);
        } catch (SQLException e) {
            e.printStackTrace();
            return null;
        }
